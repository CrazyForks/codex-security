...
deletions, sparse-checkout paths absent from the worktree, common local credential stores/key material (including `.config`, shell histories/caches, alternate-VCS metadata, `.envrc`, Composer/Bundler/Gradle credentials, and Terraform CLI/state), Git credentials/history (including tracked nested repositories),
