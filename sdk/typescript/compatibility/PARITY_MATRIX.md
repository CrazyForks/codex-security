...
sparse-checkout paths absent from the worktree, common local credential stores/key material (including `.config`, shell profiles/histories/caches, alternate-VCS metadata, `.envrc`, Composer/Bundler/Gradle credentials, Terraform CLI/state, Databricks, dbt, Snowflake/SnowSQL, gsutil, common client RC files, and credential-shaped Docker/service-account JSON), and Git credentials/history (including incomplete tracked nested repositories)
