...
- Exhaustive scope-review receipt: `<coverage_dir>/scope_review.jsonl`
  ...
  - Reference `artifacts/03_coverage/scope_review.jsonl`, `artifacts/02_discovery/scope_inventory.jsonl`, and `artifacts/02_discovery/candidate_ledger.jsonl` together in a complete coverage surface with nonempty `id`, `label`, a valid `disposition`, and `receiptRefs`. The host verifies every path and the compact candidate ledger using an isolated verifier and Python interpreter outside all model-writable roots before completion; the interpreter's original path, resolved parent, and resolved executable must all remain outside writable state. The finalizer includes all three authoritative inputs in the sealed scan artifacts. A prompt-only standard scan must run `generate_rank_input.py verify-scope-coverage` against the existing authoritative inventory and require success before running the finalizer.
...
- Compact combined candidate ledger: `<discovery_dir>/candidate_ledger.jsonl`
  ...
  - For a deferred candidate, copy its exact attack-path `proof_gap` into `coverage.deferred[].reason`; when validation alone is deferred, copy its exact recorded `counterevidence_or_proof_gap` or `remaining_uncertainty`. Do not invent a different explanation.
