...
- Exhaustive scope-review receipt: `<coverage_dir>/scope_review.jsonl`
  ...
  - Reference `artifacts/03_coverage/scope_review.jsonl` in a complete coverage surface with nonempty `id`, `label`, a valid `disposition`, and `receiptRefs`. The host verifies every path and the compact candidate ledger using an isolated verifier staged outside all model-writable roots before completion; the finalizer includes the receipt in the sealed scan artifacts.
...
- Compact combined candidate ledger: `<discovery_dir>/candidate_ledger.jsonl`
  ...
  - Bind every resulting finding's `extensions.candidateId`, coverage surface `id`, and deferred coverage `id` to the exact `candidate_id` so the host can reconcile all phase decisions before finalization. Preserve the candidate's exact CWE ids and locations. Copy compact validation `confidence`, `confidence_rationale`, `method`, and `evidence` into canonical finding `confidence.level`, `confidence.rationale`, `validation.method`, and `validation.summary`; copy attack-path `severity`, `severity_rationale`, and `change_conditions` into `severity.level`, `severity.rationale`, and `severity.changeConditions`, and preserve `dataflow`, `reachability`, `counterevidence`, `impact`, and `likelihood` in `attackPath`.
