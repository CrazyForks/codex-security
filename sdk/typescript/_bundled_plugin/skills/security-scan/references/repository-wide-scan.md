...
The inventory excludes Git administrative directories and installed `node_modules` trees beneath each requested scope. A directly requested dependency directory remains in scope, including when its parent repository or package is also requested; only unrelated dependencies, nested Git metadata, and transitive dependency trees are excluded. Never represent those built-in exclusions as an empty exclusion list or mark an inventoried path as excluded. The SDK binds their exact scope-relative patterns and reasons into `scan.scope.excludePaths`, `coverage.excludePaths`, and `coverage.explicitExclusions` during completion. For a manual scan, bind the same authoritative exclusions into the draft documents before verifying or finalizing:
