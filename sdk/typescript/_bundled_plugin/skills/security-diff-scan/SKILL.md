...
- for local patches, read every staged Git index blob in full with `git --no-replace-objects -C <repo_root> show :<repository-relative-path>`, and separately read its working-tree version in full when it is a reviewable regular file; inspect both versions even when their sampled outlines match, when one version is binary, or when the staged file no longer exists in the working tree
