...
- for local patches, read a staged-only added file that no longer exists in the working tree from the exact index blob with `git --no-replace-objects -C <repo_root> show :<repository-relative-path>`; never replace that staged content with an unrelated working-tree file
