...
describe("runtime directories and plugin Python boundary", () => {
  ...
  test("keeps scoped scan inventories within every requested SDK path", async () => {
    const root = await temporaryDirectory();
    const repository = join(root, "repository");
    ...
    await mkdir(repository);
    await Promise.all([
      writeFile(join(repository, "first.ts"), "export {};\n"),
    ...
    await writeFile(scopesFile, JSON.stringify(["first.ts", "second.ts"]));
    ...
    const runInventory = () =>
      spawnSync(
        ...
        {
          encoding: "utf8",
          env: { ...process.env, CODEX_SECURITY_TARGET_PATHS_FILE: scopesFile },
        },
      );

    expect(runInventory().status).toBe(0);
    expect(await readFile(inventory, "utf8")).toBe("first.ts\nsecond.ts\n");

    for (const invalidScopes of [[], { path: "first.ts" }, ["../outside.ts"]]) {
      await writeFile(scopesFile, JSON.stringify(invalidScopes));
      const result = runInventory();
      expect(result.status).toBe(2);
      expect(result.stderr).toContain("generate_in_scope_files:");
    }
    expect(await readFile(inventory, "utf8")).toBe("first.ts\nsecond.ts\n");
    ...
    expect(mcpConfig.mcpServers["codex-security"]!.env_vars).toContain(
