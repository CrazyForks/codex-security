...
from the worktree, common local credential stores/key material (including `.config`, shell profiles/histories/caches, alternate-VCS metadata, `.envrc`/`.flaskenv`, Composer/Bundler/Gradle/Bazel/Bun credentials, Terraform CLI/state, Databricks, dbt, Snowflake/SnowSQL, gsutil, deployment/ML CLI stores, Sentry/authinfo, browser/keychain/password databases, common database/client RC files, and credential-shaped Docker/service-account JSON), and Git credentials/history (including incomplete tracked nested repositories) are excluded. Credential-directory roots/descendants, empty targets, Git-config includes, Git-shaped targets (including an ambiguous tracked-worktree root), and
