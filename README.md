...
from the worktree, common local credential stores/key material (including `.config`, `.envrc`, `.bundle`, and Terraform state), and Git credentials/history (including tracked nested bare repositories) are excluded. Credential-directory roots, empty targets, and
